...
describe("UserService", () => {
  ...
  describe("createUser", () => {
    ...
    it("should create a user successfully", async () => {
      ...
      mockUserModel.checkExists = jest
        .fn()
        .mockResolvedValue({ exists: false, user: null });
      jest
        .spyOn(mockAuthUtils, "hashPassword")
    ...
    it("should throw error if user email already exists", async () => {
      ...
      mockUserModel.checkExists = jest
        .fn()
        .mockResolvedValue({ exists: true, user: existingUser });
  ...
  describe("authenticateUser", () => {
    ...
    it("should authenticate user successfully", async () => {
      ...
      jest.spyOn(mockAuthUtils, "correctPassword").mockResolvedValue(true);
      mockUserModel.updateLastLogin = jest
        .fn()
      ...
      // Assert
      expect(mockUserModel.findByEmail).toHaveBeenCalledWith(email, true);
      expect(mockAuthUtils.correctPassword).toHaveBeenCalledWith(
        password,
        mockUser.password,
      );
    ...
    it("should throw error if password is incorrect", async () => {
      ...
      mockUserModel.findByEmail = jest.fn().mockResolvedValue(mockUser);
      jest.spyOn(mockAuthUtils, "correctPassword").mockResolvedValue(false);
  ...
  describe("updateUser", () => {
    ...
    it("should update user with phone that exists for another user", async () => {
      // Arrange
      const existingUser = {
        id: "1",
        email: "test@example.com",
  ...
  describe("getAllUsers", () => {
    it("should return paginated users list", async () => {
      ...
      mockUserModel.findMany = jest.fn().mockResolvedValue(mockResponse);

      // Act
      const result = await UserService.getAllUsers(filters);

      // Assert
      expect(mockUserModel.findMany).toHaveBeenCalledWith(filters);
